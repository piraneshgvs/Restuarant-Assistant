...
public class CheifImplDAO implements CheifDAO {
	...
	public List<List<Orders>> showTable() {
		...
		/*
		 * String user_id = null; String query1 =
		 * "select distinct user_id from cart where tabno=(?) and status='no'";
		 * List<Map<String, Object>> userid = jdbcTemplate.queryForList(query1,id); for
		 * (Map<String, Object> user : userid) {
		 * 
		 * user_id = (String) user.get("user_id"); break; }
		 * 
		 * 
		 * CutomerId.setCutid(user_id);
		 * System.out.println("inside"+CutomerId.getCutid()); String query =
		 * "select fid, fQua from cart where user_id=(?) and status='no'";
		 * List<Map<String, Object>> foodRows =
		 * jdbcTemplate.queryForList(query,user_id);
		 * 
		 * for (Map<String, Object> foodRow : foodRows) {
		 * 
		 * int a = Integer.parseInt(String.valueOf(foodRow.get("fid"))); int c =
		 * Integer.parseInt(String.valueOf(foodRow.get("fQua")));
		 * 
		 ...
		 * 
		 * String fName = (String) foodName.get("food_name"); System.out.println(fName);
